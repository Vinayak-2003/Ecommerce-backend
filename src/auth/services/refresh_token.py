...
from utilities.token_creation import create_access_token, create_new_refresh_token
from utilities.logger_middleware import get_logger

logger = get_logger(__name__)
settings = get_settings()

async def create_refresh_access_token(request: Request, response: Response, db_session: AsyncSession):
    try:
        async with db_session.begin():
            ...
            if refresh_token_details_from_db is None:
                logger.error(f"This is an invalid refresh token")
                raise HTTPException(
            ...
            if refresh_token_details_from_db.expires_at < current_datetime:
                logger.error(f"The refresh token has expired")
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            if refresh_token_details_from_db.revoked:
                logger.error(f"The refresh token has been revoked")
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            user_details_query = await db_session.execute(select(User).filter(
                User.user_id == refresh_token_details_from_db.user_id
            ))
            user_details = user_details_query.scalar_one_or_none()

            if user_details is None:
                logger.error(f"User associated with the refresh token does not exist")
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        logger.exception(f"Unexpected error during refresh token: {str(e)}")
        raise HTTPException(
