...
async def create_cart_current_user(
    create_cart_details: CartItemCreate, token: str, db_session: AsyncSession
):
    """
    ...
    try:
        ...
        product_price_query = await db_session.execute(
        ...
        cart_item_query = await db_session.execute(
        ...
        if cart_item is not None:
            ...
            logger.info("Existed cart details fetched and updated")
        else:
            ...
            logger.info("Created a new cart item")
        ...
        raise HTTPException(
            status_code=500, detail=f"An error occurred while creating cart: {str(e)}"
        )
