...
def fetch_all_orders(token: Annotated[str, Depends(oauth2_scheme)],
                     page_no: int = Query(default=1, ge=1),
                     per_page: int = Query(default=10, ge=1, le=100),
                     db: Session = Depends(get_db_session)
                    ):
    return fetch_all_paginated_orders(token, page_no, per_page, db)
...
def fetch_order_by_id(order_id: str,
    ...
    return fetch_current_user_order_by_id(order_id, token, db)
...
def create_order(new_order: OrderCreate,
    ...
    return create_order_current_user(new_order, token, db)


# can be access by admins only 
@order_route.put("/update-status")
def update_order_status(update_status: OrderUpdate,
                        token: Annotated[str, Depends(oauth2_scheme)],
                        db: Session = Depends(get_db_session)
                    ):
    return update_user_order_status(update_status, token, db)


@order_route.delete("/cancel-order/me/{order_id}")
def cancel_order(token: Annotated[str, Depends(oauth2_scheme)],
                 order_id: str,
                 db: Session = Depends(get_db_session)
                ):
    return delete_current_user_order(token, order_id, db)