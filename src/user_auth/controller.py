...
async def fetch_active_user(token: Annotated[str, Depends(oauth2_scheme)],
                      db: AsyncSession = Depends(get_db_session)
                    ):
...
async def update_current_user_details(token: Annotated[str, Depends(oauth2_scheme)],
...
async def update_user_role_details(email: str,
